...
public class GuiSymbols {
    ...
    public static void open(Player player, BiConsumer<Boolean, @Nullable Symbols> consumer) {
        ...
        for (var symbol : Symbols.getSymbols()) {
            dialog.addButton(ButtonSpec.of(CustomColor.PURE_RANDOM.getHex() + symbol.getSymbol(), symbol.getName()).setDescription("Click para elegir"));
        }
        dialog.open();
    }
}
