...
import ex.api.base.utils.UtilsManagers;
...
public class RequestInvite {
    ...
    public RequestInvite() {
        ...
        this.utilsManagers = Ex.getUtilsManagers();
        this.start
                ();
    }
            ...
            for (var invites : new ArrayList<>(invites)) {
                if (invites.isExpired()) {
                    ...
                    if (request != null && request.isOnline()) {
                        utilsManagers.sendMessage(request, "%prefix% &cTu invitación ha expirado.");
                    }

                    Player receiver = Bukkit.getPlayer(invites.receiver());
                    if (receiver != null && receiver.isOnline()) {
                        utilsManagers.sendMessage(receiver, "%prefix% &cTu invitación ha expirado.");
                    }
